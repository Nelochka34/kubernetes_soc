...
#### 2. Настройка IP (пул IP), которые MetalLB может раздать
 (metallb-config.yaml)
...
#### 4. Настройка LoadBalancer Service. 
Сервис говорит Kubernetes, что нужен внешний IP, Metallb берет IP из vector-pool, назначает IP. 
